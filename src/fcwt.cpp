...
namespace fcwt {
    ...
    void create_optimization_schemes(int maxsize, int threads, int flags) {
        ...
        if(nt <= 10) {
            Rcpp::Rcerr << "Maxsize is too small (<=1024)... please use a larger number\n";
        ...
        for(int i=11; i<=nt; i++) {
            ...
            Rcpp::Rcout << "Calculating optimal scheme for forward FFT with N:" << n << "\n";
            p_for = fftwf_plan_dft_r2c_1d(n, dat, O1, flags);
            fftwf_plan_dft_r2c_1d(n, dat, O1, flags);

            // std::cout << "Calculating optimal scheme for backward FFT with N:" << n << std::endl;
            Rcpp::Rcout << "Calculating optimal scheme for backward FFT with N:" << n << "\n";
            p_back = fftwf_plan_dft_1d(n, O1, out, FFTW_BACKWARD, flags);
            fftwf_plan_dft_1d(n, O1, out, FFTW_BACKWARD, flags);
            ...
            Rcpp::Rcout << "Optimization schemes for N: " << n << " have been calculated. Next time you use fCWT it will automatically choose the right optimization scheme based on number of threads and signal length.\n";
        }
    }
    ...
    void load_optimization_schemes(bool use_optimalization_schemes, int size, int nthread) {
        if(use_optimalization_schemes) {
            if(size <= 1024) {
                ...
                Rcpp::Rcout << "Inputsize is too small (N <= 1024) to use optimization.\n";
                return;
            }
            ...
            if(!fftwf_import_wisdom_from_filename(file_for)) {
                ...
                Rcpp::Rcout << "WARNING: Optimization scheme was not found, fallback to calculation without optimization.\n";
            }
        }
    }
                     ...
                     int *pinputsize, float *pcenterfrequency, int nthreads, bool use_optimalization_schemes)
    {
        ...
        //for later use of avx-instructions we need 32 byte alignment.
